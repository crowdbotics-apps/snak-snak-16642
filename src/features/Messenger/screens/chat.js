...
import {Button, Text, Input, Avatar, withStyles} from 'react-native-ui-kitten';
...
export class _Chat extends React.Component {
  ...
  static renderNavigationTitle = (navigation, user) => (
    <TouchableOpacity
      onPress={() => _Chat.onNavigationTitlePressed(navigation, user)}>
      <View style={{alignItems: 'center'}}>
        <Text category="s1" style={{color: 'black'}}>{`${user.firstName} ${
          user.lastName
        }`}</Text>
  ...
  static renderNavigationAvatar = (navigation, user) => (
    <TouchableOpacity
      onPress={() => _Chat.onNavigationAvatarPressed(navigation, user)}>
  ...
  renderItem = ({item}) => {
    ...
    const itemStyle = isIncoming
      ? this.props.themedStyle.itemIn
      : this.props.themedStyle.itemOut;
      ...
      <KeyboardAwareScrollView
        ...
        <View style={this.props.themedStyle.footer}>
          ...
          <TouchableOpacity
            ...
            style={this.props.themedStyle.send}>
